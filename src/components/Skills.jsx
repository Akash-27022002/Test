...
const SectionCard = ({ title, icon: Icon, items }) => (
  <div className="rounded-xl border border-neutral-800 bg-neutral-900/40 p-5">
    ...
    <div className="mt-4 flex flex-wrap gap-2">
      {items.map((it) => (
        <span key={it} className="px-2.5 py-1 text-xs rounded-md bg-neutral-800/70 text-neutral-200 border border-neutral-700">{it}</span>
      ))}
    </div>
  </div>
);
...
export default function Skills({ data }) {
  return (
    <section id="skills" className="py-12 sm:py-16 border-t border-neutral-900/60">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight text-white">Skills</h2>
          <span className="text-xs text-neutral-400">Constantly learning</span>
        </div>
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-6">
          {data.strengths.map((s) => (
            <div key={s.label} className="rounded-xl border border-neutral-800 bg-neutral-900/40 p-5">
              ...
              <div className="mt-3 h-2 w-full rounded-md bg-neutral-800">
